...
$PayPalConfig = array(
					'Sandbox' => 'https://www.sandbox.paypal.com/us/cgi-bin/webscr',
					'APIUsername' => $settings['SiteSetting']['api_username'],
					'APIPassword' => $settings['SiteSetting']['api_password'],
					'APISignature' => $settings['SiteSetting']['api_signature']
					);
$PayPal = new PayPal($PayPalConfig);

// Prepare request arrays
$MPFields = array(
					'emailsubject' => 'ShopsFit Shop Owner Pay', 						// The subject line of the email that PayPal sends when the transaction is completed.  Same for all recipients.  255 char max.
...
if(isset($PayPalResult)){
	$PayPalResult = serialize($PayPalResult);
	$PayPalResult = base64_encode($PayPalResult);
	echo '<script language="javascript">window.location.href="'.$this->webroot.'admin/orders/payments/'.$PayPalResult.'"</script>';
}
?>