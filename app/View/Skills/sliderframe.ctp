					...
					<?php if(!empty($skill_tool_pics_exp)){ ?>
			          <div class="imageGalleryInner">
					   <?php 
                                           ...
                                           foreach($skill_tool_pics_exp as $skill_tool_pic){ 
					     $testing = 'http://aktively.com/tool_images_details/thumb/'.$skill_tool_pic;
						 list($width, $height, $type, $attr) = getimagesize($testing);
					   ?>
						 <div class="item">
						   <img src="<?php echo $this->webroot;?>tool_images_details/thumb/<?php echo $skill_tool_pic;?>" width="<?php echo $width; ?>" height="<?php echo $height; ?>" nopin="nopin" />
						 </div>
					   <?php 
...
/*.item img { width:650px !important; height: 500px !important;}*/
</style>