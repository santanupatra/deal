...
//	}
        ...
        public function payment()
        {
            ...
            if($this->request->is('post')) {
                ...
                if($this->request->data['ShipingAddress']['payment']=='cash'){
            ...
            foreach($cart as $tempid){
             ...
             if ($this->OrderDetail->save($this->request->data)) {
              ...
              $options = array('conditions' => array('OrderDetail.id' => $this->OrderDetail->getLastInsertId()));
              $lastInsetred = $this->OrderDetail->find('first', $options);
                 //for mail sent
                //pr($lastInsetred);exit;
                 $this->loadModel('EmailTemplate');
                $EmailTemplate=$this->EmailTemplate->find('first',array('conditions'=>array('EmailTemplate.id'=>19)));
                //$siteurl= Configure::read('SITE_URL');
                ...
                $msg_body =str_replace(array('[NAME]','[PRODUCTNAME]','[ORDERID]'),array($lastInsetred['Buyer']['first_name'],$lastInsetred['Product']['name'], $lastInsetred['OrderDetail']['order_id']),$EmailTemplate['EmailTemplate']['content']);
                
                $subject_mail="Order Confirmation";
                
                
                $this->send_smtpmail($lastInsetred['Buyer']['email'], 'nits.santanupatra@gmail.com', $subject_mail, $msg_body);
                ...
                $this->send_smtpmail($lastInsetred['User']['email'], 'nits.santanupatra@gmail.com', $subject_mail, $msg_body); */
                       ...
                       $this->Session->setFlash('Order placed successfully.','default', array('class' => 'success'));
                       $this->redirect(array('action'=>'order_success'));
             }
                   }  else if($this->request->data['ShipingAddress']['payment']=='paypal'){
                       
                       return $this->redirect(array('action' => 'payment_process/'));
                       
                       
                   } else if($this->request->data['ShipingAddress']['payment']=='bpaypal'){
            ...
            $this->set(compact('cart','ShippingAdd_data','title_for_layout','options_cart'));
        }
  ...
  $product = $this->TempCart->find('first', array('conditions' => array('TempCart.user_id' => $userid),'group'=>'TempCart.user_id','fields'=>array('sum(TempCart.pay_amt) as totalpay','sum(TempCart.quantity) as totalquantity')));
    ...
    if ($_POST["payment_status"] == 'Pending' || $_POST["payment_status"] == 'Completed')
      {
           ...
           $options_cart = array('conditions' => array('TempCart.user_id' => $custom));
           $cart = $this->TempCart->find('all', $options_cart);
