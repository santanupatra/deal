...
function myinbox() { 
    ...
    try {
        ...
        $sql = "SELECT * FROM comments WHERE to_user_id=:user_id and buyer_is_delete=0 group by thread_id ORDER BY id DESC";
        $db = getConnection();
        $stmt = $db->prepare($sql);
        $stmt->bindParam("user_id", $user_id);
        $stmt->execute();
					    ...
					    "order_id" => stripslashes($messages->order_id),
						 "order_details_id" => stripslashes($messages->order_details_id),
						  "thread_id" => stripslashes($messages->thread_id),
						   "buyer_is_delete" => stripslashes($messages->buyer_is_delete),
...
function mysent() { 
    ...
    try {
        ...
        $sql = "SELECT * FROM comments WHERE user_id=:user_id group by thread_id ORDER BY id DESC";
        $db = getConnection();
        $stmt = $db->prepare($sql);
        $stmt->bindParam("user_id", $user_id);
        $stmt->execute();
        ...
        if ($count > 0) {
			...
			$userDetails = "select * FROM users WHERE `id`='" . $messages->to_id . "'";
			$stmtuserDetails = $db->query($userDetails);
			$userData = $stmtuserDetails->fetchObject();
			$full_name=$userData->first_name.' '.$userData->last_name;
...
function messagereply() {
    ...
    try {
		...
		$comments=$comments;
		$file_name=$commentsdetails->file_name;
		$is_read=$commentsdetails->is_read;
		$folder_id=$commentsdetails->folder_id;
		$seller_is_flag=$commentsdetails->seller_is_flag;
		$seller_is_delete=$commentsdetails->seller_is_delete;
		$buyer_is_delete=$commentsdetails->buyer_is_delete;
		$buyer_is_flag=$commentsdetails->buyer_is_flag;
		$cdate= gmdate('Y-m-d H:i:s');
        $count = $stmt->rowCount();
        if ($count > 0) {
            $insertInventoryDetails = "insert into comments (user_id, to_user_id, comment_type , is_notification, order_id, order_details_id, thread_id, subject, comments, file_name, is_read,folder_id, seller_is_flag,seller_is_delete,buyer_is_delete,buyer_is_flag, cdate) values(:user_id,:to_user_id,:comment_type,:is_notification,:order_id,:order_details_id,:thread_id,:subject,:comments,:file_name,:is_read,:folder_id,:seller_is_flag,:seller_is_delete,:buyer_is_delete,:buyer_is_flag,:cdate)";
		...
		$stmt->bindParam("buyer_is_flag", $buyer_is_flag);
		$stmt->bindParam("cdate", $cdate);
		$stmt->execute();	
...
function addfeedback() {
  ...
  function cancelorder() {
	...
	$sql1 = "UPDATE order_details SET user_type=:user_type,cancel_user_id=:user_id,order_status=:order_status,cancel_id=:cancel_id  WHERE id=:order_details_id";
			...
			$subject='Product canceled by buyer for '.$select_reason.'. Order ID: '.$NewOrderId;
			$product_id=$orderData->product_id;
