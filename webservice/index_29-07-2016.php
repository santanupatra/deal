...
//    echo $paramValue = $app->request->post('fname');
        $sql = "INSERT INTO users (first_name, last_name, email, password, company_name, mobile_number, my_latitude, my_longitude) VALUES (:first_name, :last_name, :email, :password, :company_name, :mobile_number, :zone, :my_latitude, :my_longitude)";
        try {
            $db = getConnection();
            $stmt = $db->prepare($sql);
...
function forgotPassword() {
    ...
    if (!empty($byeamil)) {
        ...
        $db = null;
        $data['ACK'] = '1';
        $data['error'] = 'Mail Send Successfully';
    } else {
        $data['last_id'] = '';
        $data['ACK'] = '0';
        $data['error'] = 'Email not found in our database';
    }
...
function removeProductFromCart($userid, $productid) {
    $data = array();
    $sql = "DELETE FROM freshquater_cart WHERE userid=:userid AND productid=:productid";
    try {
        $db = getConnection();
        $stmt = $db->prepare($sql);
        $stmt->bindParam("userid", $userid);
...
function addToCart($userid, $productid, $quantity) {
    ...
    try {
        ...
        $sql = "SELECT * FROM freshquater_cart WHERE userid=:userid AND productid=:productid ORDER BY id DESC";
        $db = getConnection();
        $stmt = $db->prepare($sql);
        $stmt->bindParam("userid", $userid);
        $stmt->bindParam("productid", $productid);
        ...
        if ($count == 0) {
            ...
            $sql = "INSERT INTO freshquater_cart (userid, productid, quantity) VALUES (:userid, :productid, :quantity)";
            $db = getConnection();
            $stmt = $db->prepare($sql);
            $stmt->bindParam("userid", $userid);
            $stmt->bindParam("productid", $productid);
            ...
            if ($quantity == 0) {
                ...
                $status = 0;
                $sql = "UPDATE freshquater_cart SET quantity=:quantity WHERE userid=:userid AND productid=:productid";
                $db = getConnection();
                $stmt = $db->prepare($sql);
                $stmt->bindParam("userid", $userid);
                $stmt->bindParam("productid", $productid);
        ...
        if ($count > 0) {
            foreach ($getproducts as $products) {
                ...
                $allproducts[] = array(
                    ...
                    "desc" => stripslashes(strip_tags($productData->desc)),
                    "quantity" => stripslashes(strip_tags($products->quantity)),
...
//    echo $paramValue = $app->request->post('fname');
//exit;
    $sql = "UPDATE freshquater_user SET building_no=:building_no, zone=:zone, b_street=:b_street, b_address1=:b_address1, notes=:notes, my_latitude=:my_latitude, my_longitude=:my_longitude WHERE id=:id";
    try {
        $db = getConnection();
        $stmt = $db->prepare($sql);
        $stmt->bindParam("building_no", $building_no);
        $stmt->bindParam("zone", $zone);
        $stmt->bindParam("b_street", $b_street);
        ...
        $data['userDetails'] = array(
            ...
            "b_address1" => stripslashes($getUserdetails->b_address1),
            "b_address2" => stripslashes($getUserdetails->b_address2),
            "b_street" => stripslashes($getUserdetails->b_street),
            "b_city" => stripslashes($getUserdetails->b_city),
            "b_state" => stripslashes($getUserdetails->b_state),
            "b_country" => stripslashes($getUserdetails->b_country),
            "building_no" => stripslashes($getUserdetails->building_no),
            "zone" => stripslashes($getUserdetails->zone),
...
function getOrderList($user_id) {
    ...
    try {
        ...
        $sql = "SELECT * FROM freshquater_orders WHERE user_id=:user_id order by `id` DESC";
        $db = getConnection();
        $stmt = $db->prepare($sql);
        $stmt->bindParam("user_id", $user_id);
        $stmt->execute();
        ...
        if ($count > 0) {
            foreach ($getproducts as $products) {
                ...
                $allproducts[] = array(
                    "id" => stripslashes($products->id),
                    "order_from_device" => stripslashes($products->order_from_device),
                    "unique_trans_id" => stripslashes($products->unique_trans_id),
                    "order_date" => stripslashes($products->date),
                    "unique_trans_id" => stripslashes($products->unique_trans_id),
                    "price" => stripslashes(strip_tags($products->price)),
...
function getOederDetails($order_id) {
    $data = array();
    $allproducts = array();
    try {

        $sql = "SELECT * FROM freshquater_tblorderdetails WHERE orderid=:orderid";
        $db = getConnection();
        $stmt = $db->prepare($sql);
        ...
        if ($count > 0) {
            ...
            if ($orderUserdetails->status == "0") {
                $order_status = "Processing Order";
            }
            if ($orderUserdetails->status == "1") {
                $order_status = "Delivered";
            }
            if ($orderUserdetails->status == "2") {
                $order_status = "Completed";
            }

            $data['order_details'] = array(
                "unique_trans_id" => stripslashes($orderUserdetails->unique_trans_id),
                "order_from_device" => stripslashes($orderUserdetails->order_from_device),
                "order_date" => stripslashes($orderUserdetails->date),
                "f_name" => stripslashes($orderUserdetails->f_name),
                "l_name" => stripslashes($orderUserdetails->l_name),
                "address1" => stripslashes($orderUserdetails->address1),
                "address2" => stripslashes($orderUserdetails->address2),
                "city" => stripslashes($orderUserdetails->city),
                "zip" => stripslashes($orderUserdetails->zip),
                "order_status" => stripslashes($order_status),
                "phone" => stripslashes($orderUserdetails->phone),
                "price" => stripslashes($orderUserdetails->price));
